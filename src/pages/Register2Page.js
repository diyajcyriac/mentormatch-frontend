...
export default function Register2Page() {
  ...
  useEffect(() => {
    if (userId) {
      fetch(`https://mentormatch-backend-y3wu.onrender.com/profile/${userId}`)
  ...
  async function updateProfile(ev) {
    ...
    try {
      const response = await fetch("https://mentormatch-backend-y3wu.onrender.com/register-step-2", {
        method: "POST",
